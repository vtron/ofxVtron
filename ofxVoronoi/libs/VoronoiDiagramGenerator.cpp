...
bool VoronoiDiagramGenerator::generateVoronoi(float *xValues, float *yValues, int numPnts, float minX, float maxX, float minY, float maxY, float minDist)
{
	cleanup();
	cleanupEdges();
	int i;
...
bool VoronoiDiagramGenerator::generateVoronoi(vector <int> xValues, vector <int> yValues, int minX, int maxX, int minY, int maxY, int  minDist)
{
	cleanup();
	cleanupEdges();
	int i;
...
struct Halfedge * VoronoiDiagramGenerator::ELgethash(int b)
{
	...
	/* Hash table Pnts to deleted half edge.  Patch as necessary. */
...
struct Halfedge * VoronoiDiagramGenerator::ELleftbnd(struct Pnt *p)
{
	int i, bucket;
	struct Halfedge *he;
	...
	if (he==ELleftend  || (he != ELrightend && right_of(he,p)))
	{
		...
		{
			he = he -> ELright;
		} while (he!=ELrightend && right_of(he,p));	//keep going right on the list until either the end is reached, or you find the 1st edge which the Pnt
		he = he -> ELleft;				//isn't to the right of
	}
	else 							//if the Pnt is to the left of the HalfEdge, then search left for the HE just to the left of the Pnt
		do 
		{
			he = he -> ELleft;
...
/* This delete routine can't reclaim node, since Pnters from hash
...
struct Edge * VoronoiDiagramGenerator::bisect(struct Site *s1,struct	Site *s2)
{
	...
	ref(s1); 
	ref(s2);
	newedge -> ep[0] = (struct Site *) NULL; //to begin with, there are no endPnts on the bisector - it goes to infinity
...
//create a new site where the HalfEdges el1 and el2 intersect - note that the Pnt in the argument list is not used, don't know why it's there
struct Site * VoronoiDiagramGenerator::intersect(struct Halfedge *el1, struct Halfedge *el2, struct Pnt *p)
{
	struct	Edge *e1,*e2, *e;
	struct  Halfedge *el;
	float d, xint, yint;
	...
	//create a new site at the Pnt of intersection - this is a new vector event waiting to happen
...
int VoronoiDiagramGenerator::right_of(struct Halfedge *el,struct Pnt *p)
{
	struct Edge *e;
	struct Site *topsite;
...
void VoronoiDiagramGenerator::endPnt(struct Edge *e,int lr,struct Site * s)
{
	e -> ep[lr] = s;
	ref(s);
...
struct Pnt VoronoiDiagramGenerator::PQ_min()
{
	struct Pnt answer;
...
void VoronoiDiagramGenerator::clip_line(struct Edge *e)
{
	...
	//if the distance between the two Pnts this line was created from is less than 
...
bool VoronoiDiagramGenerator::voronoi(int triangulate)
{
	...
	while(1)
	{
		...
		{	
			...
			endPnt(lbnd->ELedge,lbnd->ELpm,v);	//set the endPnt of the left HalfEdge to be this vector
			endPnt(rbnd->ELedge,rbnd->ELpm,v);	//set the endPnt of the right HalfEdge to be this vector
			ELdelete(lbnd);							//mark the lowest HE for deletion - can't delete yet because there might be Pnters to it in Hash Map	
			PQdelete(rbnd);							//remove all vertex events to do with the  right HE
			ELdelete(rbnd);							//mark the right HE for deletion - can't delete yet because there might be Pnters to it in Hash Map	
			...
			bisector = HEcreate(e, pm);				//create a HE from the Edge 'e', and make it Pnt to that edge with its ELedge field
			ELinsert(llbnd, bisector);				//insert the new bisector to the right of the left HE
			endPnt(e, re-pm, v);					//set one endPnt to the new edge to be the vector Pnt 'v'.
													//If the site to the left of this bisector is higher than the right
													//Site, then this endPnt is put in position 0; otherwise in pos 1
...
{
	struct Pnt *s1 = (Pnt*)p1, *s2=(Pnt*)p2;
