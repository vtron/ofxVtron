...
public:
	...
	bool generateVoronoi(float *xValues, float *yValues, int numPnts, float minX, float maxX, float minY, float maxY, float minDist=0);
	bool generateVoronoi(vector <int> xValues, vector <int> yValues, int minX, int maxX, int minY, int minY, int  minDist);	
...
private:
	...
	void endPnt(struct Edge *e,int lr,struct Site * s);

	void ELdelete(struct Halfedge *he);
	struct Halfedge *ELleftbnd(struct Pnt *p);
	...
	int right_of(struct Halfedge *el,struct Pnt *p);

	struct Site *rightreg(struct Halfedge *he);
	struct Edge *bisect(struct	Site *s1,struct	Site *s2);
	float dist(struct Site *s,struct Site *t);
	struct Site *intersect(struct Halfedge *el1, struct Halfedge *el2, struct Pnt *p=0);
